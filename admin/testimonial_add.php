<?php
          ...
          if($result->num_rows > 0){
            while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)){
              ...
              $text_uz_ser = $row['bodyy_uz'];
              $title_en_ser = $row['title_en'];
              $text_en_ser = $row['bodyy_en'];
                        }} 
          ...
          try{
            ...
            if(isset($_POST['add'])){
                      $smtp = $connection->prepare("INSERT INTO testimonial (img, title_uz, title_en, bodyy_uz, bodyy_en ) VALUES ( :img, :title_uz, :title_en, :text_uz, :text_en)");
                ...
                if(isset($_POST['update'])){
                  ...
                  if($tmpname != ""){
                    $smtp = $connection->prepare("UPDATE  `testimonial` SET  img=:img , title_uz=:title_uz, title_en=:title_en, body_uz=:body_uz, body_en=:body_en WHERE id=:id");
                    $smtp->bindParam(':id', $_POST['id']);
                    $smtp->bindParam(':title_uz', $_POST['title_uz']);
                    $smtp->bindParam(':title_en', $_POST['title_en']);
                    $smtp->bindParam(':bodyy_uz', $_POST['text_uz']);
                    $smtp->bindParam(':bodyy_en', $_POST['text_en']);
                    $smtp->bindParam(':img', $filename);
                    $smtp->execute();
                    header("location: testimonial.php");
                    $success[] = "Muvaffaqiyatli yuklandi!";
          } else{
                    $smtp = $connection->prepare("UPDATE  `testimonial` SET   title_uz=:title_uz, title_en=:title_en, body_uz=:body_uz, body_en=:body_en WHERE id=:id");
                    $smtp->bindParam(':id', $_POST['id']);
                    $smtp->bindParam(':title_uz', $_POST['title_uz']);
                    $smtp->bindParam(':title_en', $_POST['title_en']);
                    $smtp->bindParam(':bodyy_uz', $_POST['text_uz']);
  ...
  color: #888;
    ...
    <body>
        ...
        <main class="container section">
                    ...
                    <h3>Portfoli add</h3>
                    <img src="img1/<?=$icon_ser?>" alt="" width="100px">
                    <fieldset>
